...
namespace mattatz.TeddySystem {
	...
	public class Teddy {
		...
		List<Face2D> Categorize (List<Triangle2D> triangles) {
			return triangles.Select(t => {
				return new Face2D(GetFaceType(t), t);
			}).ToList();
		}

		Face2DType GetFaceType (Triangle2D t) {
			int count = 0;
		...
		float Distance (Segment2D line, Vertex2D v) {
		...
		void ChordalAxisRoutine (Chord2D chord, Connection2D connection) {
		 ...
		 */
		void Prune (Chord2D chord, List<Vertex2D> convergence, Chord2D from = null, List<Chord2D> past = null) {
		...
		void Prune (List<Chord2D> chords) {
			chords.ForEach(ch => { 
				ch.Prune();
		...
		void Subdivide (Chord2D root) {

			var rTriangles = new List<Triangle2D>();
			var rSegments = new List<Segment2D>();

			Action<Chord2D, Chord2D> SubdivideRoutine;
			SubdivideRoutine = (Chord2D chord, Chord2D from) => {
				...
				chord.Connection.ForEach(to => {
					if(to != from) {
						SubdivideRoutine(to, chord);
					}
				});

			};

			SubdivideRoutine(root, null);
		...
		Dictionary<Vertex2D, VertexNetwork2D> BuildTable (Triangulation2D tri) {
		...
		List<VertexNetwork2D> BuildNetworks (Triangulation2D tri, Dictionary<Vertex2D, VertexNetwork2D> networkTable) {
		...
		*/
		void Elevate (List<VertexNetwork2D> networks) {
			for(int i = 0; i < 3; i++) {
		...
		List<Triangle2D> Sew (Dictionary<Vertex2D, float> heightTable, Triangulation2D tri, Segment2D left, Segment2D right, int division) {
			Vertex2D top, lb, rb;
		...
		float QuarterOval (float height, float distance, float r) {
			if(height >= 0f) {
		...
		List<Chord2D> Traverse (Chord2D cur, Chord2D from, Func<Chord2D, bool> check) {
		...
		float Angle (Vector2 p0, Vector2 p1) {
